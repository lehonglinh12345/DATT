<?php
$section = 'tech';
$page_title = "Kỹ Thuật Trồng Cây";
$page_desc = "Chia sẻ kỹ thuật trồng cây, nhân giống và chăm sóc cây trồng theo mùa vụ cho nhà nông.";
$active_page = 'tech';
...
if (!$selected_article) {
    ...
    // Sidebar recent planting articles
...
<section class="about-hero" style="background: linear-gradient(rgba(18, 24, 32, 0.75), rgba(18, 24, 32, 0.8)), url('images/hero-bg.jpg') center/cover;">
    <div class="container">
        ...
        <div class="breadcrumbs">
            ...
            <span>/</span>
            <span>Kỹ thuật trồng cây</span>
        </div>
    </div>
</section>

<!-- Techniques Main Section -->
<section class="section">
    <div class="container">
        <div class="text-center">
            <h2 class="section-title">Kỹ Thuật Trồng Cây</h2>
            <p class="section-subtitle">Chia sẻ kỹ thuật trồng trọt, nhân giống và chăm sóc cây trồng phù hợp từng mùa vụ từ các kỹ sư nông nghiệp.</p>
        </div>
            ...
            <div class="news-detail">
                <article class="news-article">
                    <div class="news-detail-img">
                        <img src="<?php echo htmlspecialchars($selected_article['image']); ?>" alt="<?php echo htmlspecialchars($selected_article['alt'] ?: $selected_article['title']); ?>">
                    </div>
                    <div class="news-article-meta">
                        <span><i class="fa-regular fa-calendar"></i> <?php echo htmlspecialchars($selected_article['date']); ?></span>
                        <span><i class="fa-regular fa-folder"></i> <?php echo htmlspecialchars($selected_article['category'] ?: 'Kỹ thuật trồng trọt'); ?></span>
                    ...
                    <div class="post-navigation">
                        <?php if ($prev_article): ?>
                            <a class="nav-prev" href="planting-techniques.php?article=<?php echo urlencode($prev_article['slug']); ?>">
                        ...
                        <?php if ($next_article): ?>
                            <a class="nav-next" href="planting-techniques.php?article=<?php echo urlencode($next_article['slug']); ?>">
                    ...
                    <div class="news-article-tag">
                        Bài viết này được đăng trong <a href="planting-techniques.php">Kỹ thuật trồng trọt</a>. Đánh dấu liên kết thường trực.
                    </div>
                ...
                <aside class="news-sidebar">
                    <div class="sidebar-widget">
                        <h3>Bài viết kỹ thuật khác</h3>
                        <ul class="related-posts">
                            <?php foreach ($visible_articles as $article): ?>
                                <?php if ($article['slug'] !== $selected_article['slug']): ?>
                                    <li>
                                        <a href="planting-techniques.php?article=<?php echo urlencode($article['slug']); ?>"><?php echo htmlspecialchars($article['title']); ?></a>
                ...
                <?php foreach ($visible_articles as $article): ?>
                    <div class="news-card">
                        <div class="news-img-wrapper">
                            <img src="<?php echo htmlspecialchars($article['image']); ?>" alt="<?php echo htmlspecialchars($article['alt'] ?: $article['title']); ?>">
                        </div>
                        <div class="news-body">
                            <div class="news-meta">
                            ...
                            <h3><a href="planting-techniques.php?article=<?php echo urlencode($article['slug']); ?>"><?php echo htmlspecialchars($article['title']); ?></a></h3>
                            <p class="news-desc">
                                <?php echo htmlspecialchars($article['excerpt']); ?>
                            </p>
                            <a href="planting-techniques.php?article=<?php echo urlencode($article['slug']); ?>" class="news-link">Đọc tiếp <i class="fa-solid fa-arrow-right-long"></i></a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
            ...
            <?php if ($total_pages > 1): ?>
                <div class="text-center" style="margin-top: 4rem; display: flex; justify-content: center; gap: 0.5rem; flex-wrap: wrap;">
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        ...
                        <a href="planting-techniques.php?page=<?php echo $i; ?>" style="width: 40px; height: 40px; border-radius: 50%; border: 1px solid <?php echo $isActive ? 'var(--color-primary)' : 'var(--color-border)'; ?>; display: flex; align-items: center; justify-content: center; color: <?php echo $isActive ? 'var(--color-white)' : 'var(--color-dark-muted)'; ?>; background-color: <?php echo $isActive ? 'var(--color-primary)' : 'var(--color-white)'; ?>; font-weight: <?php echo $isActive ? '700' : '600'; ?>; text-decoration: none;">
                            <?php echo $i; ?>
                        </a>
                    <?php endfor; ?>
                </div>
